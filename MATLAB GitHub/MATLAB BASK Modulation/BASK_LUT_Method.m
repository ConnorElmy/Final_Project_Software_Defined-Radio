...
textBinary = dec2bin(text, 8); %Converts to binary

[numChars, numBits] = size(textBinary); %The length of the X and Y axis gives the number of characters
...
for iChar=1:1:numChars
    for jBit=1:1:numBits

        bitValue = textBinary(iChar,jBit);

        for s = 1:samplesPerBit

            %Modulation
            %carrierSample = sinewave_array_64_int(lutIndex);
...
filename = sprintf('BASK_Integer_sin_array%d_AM_Fc%d_mHigh%.1f_mLow%.1f_Fs%d.wav', lutSize, Fc, mHigh, mLow, Fs);
